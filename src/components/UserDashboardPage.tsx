...
export default function UserDashboardPage({
    ...
    const loadUserData = async () => {
      ...
      try {
        ...
        bookingRows.forEach((item: any) => {
          const evt =
            allEvents.find((e) => e.id === item.eventId) ||
            allEvents[0] ||
            PLACEHOLDER_EVENT;
          loaded.push({
            id: item.id,
            ...
            orderId:
              item.orderId ||
              item.bookingNumber ||
              `JXB-${String(item.id).substring(0, 4).toUpperCase()}`,
            seat: item.seat || "F-12",
  ...
  const handlePasswordResetSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        setPasswordError(
          "That new password is too weak — try something longer.",
        );
        ...
        setPasswordError(
          "Could not update your password. Please sign out, sign back in, and try again.",
        );
      }
    } finally {
  ...
  const toPrintData = (pass: PersonalBooking): TicketPrintData => ({
    ...
    date:
      pass.event.fullDate || `${pass.event.date}, ${pass.event.year || "2026"}`,
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-6">
        <div className="bg-[#111111] border border-white/15 p-10 max-w-md w-full text-white">
          ...
          <h2 className="font-display font-bold text-3xl leading-[0.95] mt-6">
            Account suspended
          </h2>
          <p className="text-sm text-white/70 mt-4 leading-relaxed">
            Your account has been suspended. If you think this is a mistake,
            contact our support team and we'll look into it right away.
          </p>
          <div className="border-t border-white/15 mt-6 pt-5 space-y-3 text-sm">
            <div>
              <span className={`${overline} text-white/50 block`}>
                Account ID
              </span>
            ...
            <div>
              ...
              <span className="text-white/90">support@jazbatickets.com</span>
            </div>
          </div>
          <button
            onClick={onLogout}
  ...
  const totalPassesCount = userBookings.reduce(
    (sum, item) => sum + item.quantity,
    0,
  );
  const totalAmountSpent = userBookings.reduce(
    (sum, item) => sum + item.pricePaid,
    0,
  );
  ...
  const sectionHead = (title: string, sub: string) => (
    <div className="border-b border-black pb-4 mb-8">
      <h2 className="font-display font-bold text-2xl leading-[0.95]">
        {title}
      </h2>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-14">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-y-6 border-t border-white/15 mt-10 pt-8">
            {[
              ...
              <div
                key={s.label}
                className="pr-4 sm:px-10 sm:first:pl-0 sm:border-r sm:border-white/15 sm:last:border-r-0"
              >
                <div className={`${overline} text-white/50`}>{s.label}</div>
                <div className="font-display font-bold text-2xl sm:text-3xl mt-2">
                  {s.value}
                </div>
              </div>
            ))}
          ...
          <aside className="lg:col-span-3 lg:sticky lg:top-20">
                ...
                .map((item) => (
                  ...
                  >
                    ...
                    {typeof item.count === "number" ? (
                      <span
                        className={`text-xs font-bold px-2 py-0.5 ${activeTab === item.id ? "bg-[#ffed00] text-black" : "bg-[#f2f2f2] text-black"}`}
                      >
                        {item.count}
                      </span>
                    ) : (
            ...
            <div className="border border-[#e4e4e4] mt-6 p-5">
              ...
              <p className="text-sm text-[#222] mt-2 leading-relaxed">
                Our support team is here Mon–Fri, 10:30am–5:30pm UK time.{" "}
                <strong className="font-bold">support@jazbatickets.com</strong>
              </p>
            </div>
          </aside>
              ...
              >
                  ...
                  <div className="space-y-12">
                    ...
                    {nextBooking ? (
                      <div className="bg-black text-white p-8">
                        <span className={`${overline} text-[#ffed00]`}>
                          Your next event
                        </span>
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="flex flex-wrap gap-x-5 gap-y-1 mt-3 text-sm text-white/70">
                                <span className="flex items-center gap-1.5">
                                  ...
                                  {nextBooking.event.fullDate ||
                                    `${nextBooking.event.date}, 2026`}
                                </span>
                                <span className="flex items-center gap-1.5">
                                  <Clock className="w-3.5 h-3.5" />
                                  {nextBooking.event.time}
                                </span>
                                <span className="flex items-center gap-1.5 truncate">
                                  <MapPin className="w-3.5 h-3.5" />
                                  {nextBooking.event.location}
                                </span>
                              </div>
                            </div>
                          </div>
                      ...
                      <div className="border border-[#e4e4e4] p-10 text-center">
                        ...
                        <h3 className="font-display font-bold text-2xl leading-[0.95] mt-4">
                          No tickets yet
                        </h3>
                        <p className="text-sm text-[#666] mt-2 max-w-sm mx-auto">
                          When you book a show, your tickets will live here â€”
                    ...
                    <div>
                      {sectionHead(
                        "Latest activity",
                        "Account updates and offers, newest first.",
                      )}
                        ...
                        {[
                          {
                            ...
                            body: "Your account is set up and ready to book.",
                            time: "Today",
                          },
                          {
                            icon: Sparkles,
                            title: "18% off your next order",
                            body: "Use code JAZBA18 at checkout on any event.",
                            time: "Yesterday",
                          },
                          {
                            icon: Bell,
                            title: "Price alerts on",
                            body: "We'll email you when watchlist shows drop in price.",
                            time: "This week",
                          },
                        ].map((n) => (
                          <div
                            key={n.title}
                            className="flex items-start gap-4 py-5 border-b border-[#f2f2f2]"
                          >
                      ...
                      <div className="flex items-end justify-between border-b border-black pb-4 mb-8">
                        <div>
                          ...
                          <p className="text-sm text-[#666] mt-2">
                            Based on what you've booked and saved.
                          </p>
                        </div>
                        <button
                          onClick={onExploreEvents}
                        ...
                        {allEvents.filter((e) => !isPastEvent(e)).slice(0, 4).map((evt) => (
                          ...
                          >
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <h4 className="font-bold text-sm truncate mt-1">
                                {evt.title}
                              </h4>
                              <span className="text-sm text-[#666] mt-0.5 block">
                                From {formatPrice(evt.price)} · {evt.date}
                              </span>
                            </div>
                            <button
                ...
                {activeTab === "passes" && (
                  <div>
                    {sectionHead(
                      "My tickets",
                      "Show the barcode at the entrance â€” on screen or printed.",
                    )}
                    ...
                    {userBookings.length > 0 ? (
                      <div className="space-y-8">
                        {userBookings.map((pass) => (
                          <div
                            key={pass.id}
                            className="border-2 border-black flex flex-col md:flex-row"
                          >
                            {/* ── Main stub ── */}
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between gap-3 bg-black text-white px-6 py-3.5 flex-wrap">
                                ...
                                <span
                                  className={`${overline} bg-[#ffed00] text-black px-3 py-1.5`}
                                >
                                  {pass.tier === "elite"
                                    ? "Elite"
                                    : pass.tier === "vip"
                              ...
                              <div className="p-6">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-5 border-t border-[#f2f2f2] mt-6 pt-5 text-sm">
                                  <div>
                                    <span
                                      className={`${overline} text-[#8a8a8a] block`}
                                    >
                                      Date
                                    </span>
                                    <span className="font-bold mt-1 block">
                                      {pass.event.fullDate ||
                                        `${pass.event.date}, ${pass.event.year || "2026"}`}
                                    </span>
                                  </div>
                                  <div>
                                    <span
                                  ...
                                  <div>
                                    ...
                                    <span
                                      className="font-bold mt-1 block truncate"
                                      title={pass.event.location}
                                    >
                                      {pass.event.location}
                                    </span>
                                  </div>
                            ...
                            <div className="md:w-56 shrink-0 border-t-2 md:border-t-0 md:border-l-2 border-dashed border-black bg-[#f7f7f7] p-6 flex flex-row md:flex-col items-center justify-center gap-5 text-center">
                                ...
                                <div className="w-24 h-24 grid grid-cols-6 gap-px p-1">
                                  {Array.from({ length: 36 }).map((_, i) => {
                                    ...
                                    return (
                                      <span
                                        key={i}
                                        className={`w-full h-full ${filled ? "bg-black" : "bg-transparent"}`}
                                      />
                                    );
                                  })}
                                </div>
                              ...
                              <div>
                                <span className="font-bold text-xs tracking-[0.2em] block">
                                  #{pass.barCode.substring(0, 4)}-
                                  {pass.barCode.substring(4, 8)}
                                </span>
                                <span
                                  className={`${overline} text-[#666] mt-2 flex items-center justify-center gap-1`}
                                >
                                  <CheckCircle className="w-3.5 h-3.5" /> Valid
                                  for entry
                                </span>
                                <span
                                  className={`${overline} text-black bg-[#ffed00] px-2.5 py-1 inline-block mt-3`}
                                >
                                  Admit {pass.quantity}
                                </span>
                              </div>
                      ...
                      <div className="border border-[#e4e4e4] py-16 text-center">
                        <Compass className="w-8 h-8 mx-auto" />
                        <h4 className="font-display font-bold text-xl mt-4">
                          No tickets yet
                        </h4>
                        <p className="text-sm text-[#666] mt-2">
                ...
                {activeTab === "watchlist" && (
                  <div>
                    {sectionHead(
                      "Watchlist",
                      "Shows you're keeping an eye on. Book before they sell out.",
                    )}
                    ...
                    {watchlist.length > 0 ? (
                      <div className="border-t border-[#f2f2f2]">
                        {watchlist.map((show) => (
                          <div
                            key={show.id}
                            className="flex items-center gap-5 py-5 border-b border-[#f2f2f2]"
                          >
                            <img
                              src={show.image}
                              alt={show.title}
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <span className="text-sm text-[#666]">
                                From {formatPrice(show.price)} · {show.date}
                              </span>
                            </div>
                ...
                {activeTab === "billing" && (
                  <div>
                    {sectionHead(
                      "Orders & billing",
                      "Every order with its receipt â€” UK bookings include 20% VAT.",
                    )}
                      ...
                      <table className="w-full text-left text-sm">
                        <thead>
                          <tr className="border-b border-black">
                            <th className={`${overline} text-[#666] py-4 px-5`}>
                              Order
                            </th>
                            <th className={`${overline} text-[#666] py-4 px-5`}>
                              Event
                            ...
                            <th
                              className={`${overline} text-[#666] py-4 px-5 text-center`}
                            >
                              Qty
                            </th>
                            <th
                              className={`${overline} text-[#666] py-4 px-5 text-right`}
                            >
                              Amount
                            </th>
                            <th
                        ...
                        <tbody>
                          {userBookings.length > 0 ? (
                            userBookings.map((bk) => (
                              <tr
                                key={bk.id}
                                className="border-b border-[#f2f2f2] last:border-b-0 hover:bg-[#f7f7f7] transition-colors"
                              >
                                ...
                                <td className="py-4 px-5 text-center">
                                  <span className="inline-block text-xs font-bold border border-[#e4e4e4] px-2.5 py-1">
                                    Paid
                                  </span>
                                </td>
                                <td className="py-4 px-5 text-right text-[#666]">
                                  {bk.bookingDate}
                                </td>
                              </tr>
                            ))
                            ...
                            <tr>
                              <td
                                colSpan={6}
                                className="py-10 px-5 text-center text-[#666]"
                              >
                                No orders yet â€” your receipts will appear here
                    ...
                    <div className="bg-[#f7f7f7] p-5 mt-6">
                      ...
                      <p className="text-sm text-[#222] mt-2 leading-relaxed">
                        Need a VAT invoice or a business account? Email{" "}
                      ...
                      <form onSubmit={handleProfileSave}>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                          {[
                            ...
                            <div key={f.label}>
                              <label
                                className={`${overline} text-[#666] block mb-2`}
                              >
                                {f.label}
                              </label>
                        ...
                        <div className="border-t border-[#f2f2f2] mt-10 pt-8">
                          ...
                          <div className="mt-5 space-y-5">
                            {[
                              {
                                label: "Order confirmations & receipts",
                                note: "Email your tickets and receipt after every booking.",
                                checked: emailAlerts,
                                set: setEmailAlerts,
                              },
                              {
                                label: "SMS event reminders",
                                note: "A text when doors open on show day.",
                                checked: smsAlerts,
                                set: setSmsAlerts,
                              },
                              {
                                label: "Artist announcements",
                                note: "New tour dates from artists you follow.",
                                checked: artistUpdates,
                                set: setArtistUpdates,
                              },
                            ].map((p) => (
                              <label
                                key={p.label}
