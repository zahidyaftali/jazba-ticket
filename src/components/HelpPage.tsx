...
export default function HelpPage({
  ...
  const [activeTab, setActiveTab] = useState<
    "tickets" | "organisers" | "artists" | "tech"
  >("tickets");
  ...
  const knowledgebaseArticles = {
    tickets: [
      ...
      {
        id: 2,
        title: "Can I cancel or reschedule my ticket after booking?",
        desc: "Yes, within limits — see our full Refund Policy. In short: you can cancel free of charge within 24 hours of booking (as long as the event is still 72+ hours away), or up to 7 days before the event for a refund minus a 10% admin fee. Bookings made within 7 days of the event are final sale.",
      },
      {
        id: 3,
        title: "How do I know if my ticket is confirmed?",
        desc: "As soon as payment goes through, you'll see an on-screen confirmation with a reference ID, followed by an email containing your digital ticket, receipt, and QR/barcode.",
      },
      {
        id: 4,
        title: "Are group discounts available for bulk ticket purchases?",
        desc: "Yes, for most concerts, theatre shows, and conferences. For bookings of 10 or more tickets, look for a Group Discount option at checkout, or contact support@jazbatickets.com directly.",
      },
      {
        id: 5,
    ...
    organisers: [
      {
        ...
        desc: "Create a free account, select Create Event, and follow the guided setup — event details, ticket types, pricing, and capacity. Most events go live shortly after submission; some categories are manually reviewed first.",
      },
      {
        id: 7,
      ...
      },
      {
        ...
        desc: "Payouts are processed a few business days after your event date, direct to your registered bank account.",
      },
    ],
    artists: [
      {
        id: 9,
        title: "How does artist booking work?",
        desc: "Go to Book an artist, view an artist's profile and availability, and submit an enquiry with your event date, venue, and budget. You'll get a formal quote and booking agreement back, usually within 2–3 working days.",
      },
      {
        id: 10,
        title: "Is payment required upfront to book an artist?",
        desc: "A non-refundable deposit confirms a hold on the artist's date, with the balance due before the performance, per the artist's booking terms.",
      },
    ],
    tech: [
      {
        ...
        desc: 'Use "Forgot password" on the sign-in page, or change it anytime from Account settings in your dashboard.',
      },
      {
        id: 12,
        title: "My ticket barcode won't load",
        desc: "Tickets are saved for offline access automatically, and you can download a PDF copy from your dashboard in case there's no signal at the venue.",
      },
      {
        id: 13,
        title: "Can I add extra security to my account?",
        desc: "Two-factor authentication is on the way. Until then, a strong unique password keeps your account safe.",
      },
    ],
  };
  ...
  const handleTroubleshoot = (topic: string) => {
    ...
    setTimeout(() => {
      ...
      if (topic === "spoof") {
        setDiagnosticResult(
          "All ticket scanners at our partner venues are online and working normally.",
        );
      } else if (topic === "login") {
        setDiagnosticResult(
          "Sign-in is running normally. Clear your browser cache and try again, or reset your password from the login page.",
        );
      } else if (topic === "refund") {
        setDiagnosticResult(
          'Refunds are processing normally. You can cancel free within 24 hours of booking, or up to 7 days before the event (minus a 10% admin fee) — open your dashboard and choose "Request refund".',
        );
      } else {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setChatLog((prev) => [
      ...prev,
      { sender: "user", text: userMsg, time: nowStr },
    ]);
    setChatMessage("");
    setIsTyping(true);
    ...
    setTimeout(() => {
      setIsTyping(false);
      let reply =
        "Thanks for the message — a member of the team will reply shortly. You can check your tickets anytime from your dashboard.";
      if (
        userMsg.toLowerCase().includes("refund") ||
        userMsg.toLowerCase().includes("cancel")
      ) {
        reply =
          "You can cancel free within 24 hours of booking, or up to 7 days before the event for a refund minus a 10% admin fee. I've flagged this for our refunds team.";
      } else if (
        ...
        userMsg.toLowerCase().includes("ticket") ||
        userMsg.toLowerCase().includes("code") ||
        userMsg.toLowerCase().includes("qr")
      ) {
        reply =
          'Your tickets live under "My tickets" in your dashboard — just show the barcode at the entrance.';
      }
      setChatLog((prev) => [
        ...prev,
        {
  ...
  const filteredArticles = Object.values(knowledgebaseArticles)
    .flat()
    .filter(
      (art) =>
  ...
  const articles = searchQuery.trim()
    ? filteredArticles
    : knowledgebaseArticles[activeTab];
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-20 sm:py-28 relative z-10">
          ...
          <p className="text-white/70 text-base sm:text-lg mt-6 max-w-2xl leading-relaxed">
            Whether you're buying a ticket, listing an event, or booking an
            artist, this is where to find answers fast. Can't find what you
            need? Contact us at support@jazbatickets.com or +44(0)3335777014
            (Mon–Fri, 10:30am–5:30pm UK time).
          </p>
        ...
        <div className="border-b border-[#f2f2f2] mt-2">
          {articles.length > 0 ? (
            articles.map((art) => (
              <div
                key={art.id}
                className="py-6 border-b border-[#f2f2f2] last:border-b-0"
              >
                <h3 className="font-display font-bold text-lg leading-tight">
                  {art.title}
                </h3>
                <p className="text-sm text-[#666] leading-relaxed mt-2 max-w-3xl">
                  {art.desc}
                </p>
              </div>
            ))
            ...
            <div className="py-14 text-center">
              ...
              <p className="text-sm text-[#666] mt-2">
                Try different words, or ask us directly below.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-px bg-[#e4e4e4] border border-[#e4e4e4]">
          ...
          <div className="bg-white p-8 sm:p-10">
            ...
            <p className="text-sm text-[#666] mt-3 leading-relaxed">
              Pick a topic and we'll check the live system status — most issues
              resolve in seconds.
            </p>
            ...
            <AnimatePresence mode="wait">
              {diagnosticLoading && (
                ...
                >
                  <Loader2 className="w-4 h-4 animate-spin shrink-0" /> Checking
                  live status…
                </motion.div>
              )}
              ...
              <div className="flex items-center justify-between py-5 border-b border-white/15">
                <span className="flex items-center gap-3 text-sm">
                  ...
                  <span>
                    ...
                    <span className="text-white/60">
                      Billing & urgent queries
                    </span>
                  </span>
                </span>
                <span className="font-bold text-sm">+44(0)3335777014</span>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-5">
            ...
            <p className="text-sm text-[#666] mt-4 leading-relaxed max-w-sm">
              Send us the details and a support agent will reply to your email
              during support hours — Mon–Fri, 10:30am–5:30pm UK time.
            </p>
              ...
              <div className="flex items-start gap-3 py-5 border-b border-[#f2f2f2]">
                ...
                <div>
                  ...
                  <span className="text-sm text-[#666]">
                    Every barcode is issued and verified by us.
                  </span>
                </div>
              </div>
              <div className="flex items-start gap-3 py-5 border-b border-[#f2f2f2]">
            ...
            <div className="border border-black p-6 sm:p-8">
              {formSubmitted ? (
                <div className="py-10 text-center">
                  ...
                  <h3 className="font-display font-bold text-2xl leading-[0.95] mt-5">
                    Message sent
                  </h3>
                  <p className="text-sm text-[#666] mt-3 max-w-sm mx-auto">
                    Thanks — we'll reply to {contactForm.email || "your email"}{" "}
                ...
                <form onSubmit={handleContactSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                      <label className={`${overline} text-[#666] block mb-2`}>
                        Your name
                      </label>
                      <input
                        type="text"
        ...
        {chatOpen && (
          ...
          >
              ...
              {chatLog.map((msg, i) => (
                ...
                >
                  ...
                  >
                    {msg.text}
                    <span
                      className={`block text-[10px] mt-1.5 ${msg.sender === "user" ? "text-white/50" : "text-[#8a8a8a]"}`}
                    >
                      {msg.time}
                    </span>
                  </div>
            ...
            <form
              ...
              className="flex items-stretch border-t border-black shrink-0"
            >
              <input
                type="text"
                value={chatMessage}
